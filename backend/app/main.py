...
def pulse_homelab() -> dict:
    ...
    if not base_url:
        return {"available": False, "status": "Unknown", "resources": [], "docker_hosts": [], "last_updated_at": "Unknown"}
    ...
    for page in pages:
        for resource in page.get("data", []):
            ...
            resources.append({"id": pulse_value(resource.get("id")), "name": pulse_value(resource.get("name")), "type": pulse_value(resource.get("type")), "status": pulse_value(resource.get("status")), "parent_name": pulse_value(resource.get("parentName")), "last_seen": pulse_value(resource.get("lastSeen")), "updated_at": pulse_value(resource.get("updatedAt")), "runtime": pulse_value(docker.get("runtime")), "runtime_version": pulse_value(docker.get("runtimeVersion"))})

    docker_hosts = [{"name": pulse_value(host.get("name")), "containers": host.get("containers") if isinstance(host.get("containers"), int) else "Unknown", "uptime_seconds": host.get("uptimeSeconds") if isinstance(host.get("uptimeSeconds"), int) else "Unknown", "cpu_usage_percent": host.get("cpuUsagePercent") if isinstance(host.get("cpuUsagePercent"), (int, float)) else "Unknown"} for host in summary.get("dockerHosts", []) if isinstance(host, dict)]
