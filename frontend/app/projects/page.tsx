...
export default async function ProjectsPage() {
  const response = await fetch("http://backend:8000/api/projects", { cache: "no-store" }).catch(() => null);
  const projectsAvailable = response?.ok ?? false;
  ...
  return <main className="mx-auto max-w-[1600px] px-margin-mobile py-space-lg md:px-margin-desktop" aria-label="Linear and projects tracker">
    ...
    {!projectsAvailable && <StitchUnavailable className="mt-5" title="Projectregistry unavailable" detail="De lokale projectbron kon niet worden geladen. Projectkaartjes en detailroutes zijn daarom niet beschikbaar." />}
    <section className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4" aria-label="Project summary metrics"><StitchMetric label="ACTIVE PROJECTS" value={projectsAvailable ? String(active.length) : "Unavailable"} detail={projectsAvailable ? active.length ? active.map((project) => project.product_key).join(" · ") : "Geen actieve lokale projecten" : "Lokale projectbron niet beschikbaar"} tone="tertiary" /><StitchMetric label="SPRINT VELOCITY" value="Unavailable" detail="HYD-160 cycle-data ontbreekt" /><StitchMetric label="AVG BURN RATE" value="Unavailable" detail="Geen issue-tijdreeksbron" /><StitchMetric label="ACTIVE PRS" value="Unavailable" detail="Geen PR read-model" /></section>
