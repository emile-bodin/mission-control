...
export default async function BriefingsPage() {
  const response = await fetch("http://backend:8000/api/briefings", { cache: "no-store" }).catch(() => null);
  const briefingsAvailable = response?.ok ?? false;
  const latest: Briefing | undefined = response?.ok ? (await response.json())[0] : undefined;
  const briefing = latest?.briefing;
  const proposalsResponse = latest ? await fetch(`http://backend:8000/api/briefing-proposals?briefing_id=${latest.id}`, { cache: "no-store" }).catch(() => null) : null;
  const proposalsAvailable = proposalsResponse?.ok ?? false;
  const proposals: Proposal[] = proposalsResponse?.ok ? await proposalsResponse.json() : [];
  const highlights = briefing?.facts.slice(0, 3) ?? [];
  ...
  return <main className="mx-auto max-w-[1600px] px-margin-mobile py-space-lg md:px-margin-desktop" aria-label="Executive daily briefing">
    {!briefingsAvailable && <StitchUnavailable title="Briefingbron unavailable" detail="De briefing-run kon niet worden geladen." />}
    {latest && !proposalsAvailable && <StitchUnavailable className="mt-4" title="Voorstelbron unavailable" detail="Voorstelreview kan niet worden geladen." />}
    <section className="relative overflow-hidden rounded-md border border-surface-container-highest bg-surface-container p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]"><div className="absolute inset-x-0 top-0 h-24 bg-[radial-gradient(ellipse_at_top_left,rgba(76,215,246,0.16),transparent_60%)]" /><div className="relative grid gap-5 xl:grid-cols-12"><div className="xl:col-span-8"><div className="flex flex-wrap items-center gap-2"><span className="rounded bg-primary/10 px-2 py-1 font-label-caps text-label-caps text-primary">DAILY BRIEF</span><span className="font-mono text-mono-data-sm text-outline">{latest?.finished_at ? formatTime(latest.finished_at) : "NOT RUN"}</span></div><h1 className="mt-4 font-headline text-headline-xl text-on-surface">Dagbriefing</h1><p className="mt-2 max-w-3xl text-body-lg text-on-surface-variant">{briefing?.summary ?? "Geen geldige briefing beschikbaar. Alleen feitelijke bronnen en onbekenden worden getoond."}</p><div className="mt-5 grid gap-3 md:grid-cols-3" aria-label="Gevalideerde briefing highlights">{highlights.map((fact, index) => <BriefingFactCard key={fact} index={index + 1} detail={fact} />)}{!highlights.length && <BriefingFactCard index={1} detail="Geen gevalideerd briefingfeit beschikbaar." />}</div></div><div className="flex gap-3 xl:col-span-4 xl:flex-col xl:justify-center"><div className="flex-1"><BriefingRefresh /></div><Link className="cortex-focus flex flex-1 items-center justify-center rounded bg-surface-container-high px-4 py-3 text-body-sm text-on-surface" href="/actions"><span className="material-symbols-outlined mr-2 text-primary" aria-hidden="true">checklist</span>Bekijk work-items</Link></div></div></section>
...
interface BriefingFactCardProps { readonly index: number; readonly detail: string; }

function BriefingFactCard({ index, detail }: BriefingFactCardProps) {
  return <article className="rounded bg-surface-container-low p-3"><span className="rounded bg-primary/10 px-2 py-1 font-mono text-mono-data-sm text-primary">FACT {String(index).padStart(2, "0")}</span><p className="mt-3 line-clamp-4 text-body-sm text-on-surface-variant">{detail}</p></article>;
}
