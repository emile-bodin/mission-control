...
const navigation = [["Today", "/", true], ["Projects", "/projects", true], ["Status Cards", "/status-cards", true], ["Actions", "", false], ["Codex Runs", "", false], ["Homelab", "", false], ["Settings", "", false]] as const;
...
export default async function TodayPage() {
  ...
  const backendOk = projectsResponse.ok && cardsResponse.ok;
    ...
    <div className="min-h-screen bg-[#070b12] text-slate-100">
      ...
      <main className="md:pl-64">
        ...
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
          ...
          <section className="mt-7 grid gap-px overflow-hidden border border-slate-800 bg-slate-800 sm:grid-cols-2 xl:grid-cols-4" aria-label="Mission summary"><Summary label="Active projects" value={String(activeProjects.length)} detail={`${visibleProjects.length} geregistreerd`} /><Summary label="Open status cards" value={String(openCards.length)} detail="handmatig beheerd" /><Summary label="Need attention" value={String(needsAttention.length)} detail="actie nodig + geblokkeerd" /><Summary label="External integrations" value="Disabled" detail="status: Unknown" /></section>
          ...
          <section className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <Panel title="Activity" detail="Recente open statuskaarten"><div className="space-y-3">{recentCards.map((card) => <Link className="block border-l-2 border-cyan-400/50 bg-slate-950/50 px-3 py-2 hover:bg-slate-800" href={`/status-cards/${card.id}`} key={card.id}><p className="text-xs text-cyan-300">{card.status} · {card.source_type}</p><p className="mt-1 text-sm text-slate-200">{card.title}</p><p className="mt-1 text-xs text-slate-500">{card.interpretation}</p></Link>)}{!recentCards.length && <p className="text-sm text-slate-500">Geen open lokale statuskaarten.</p>}</div></Panel>
          </section>
