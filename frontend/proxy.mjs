...
function forwardedClientIp(headers, trusted) {
  if (!trusted) return undefined;
  const value = headers["x-forwarded-for"];
  const first = Array.isArray(value) ? value[0] : value?.split(",")[0];
  const address = normaliseAddress(first?.trim());
...
export function prepareRequest(request, config) {
  ...
  const clientIp = forwardedClientIp(sourceHeaders, trusted);

  for (const name of ["x-forwarded-proto", "x-forwarded-for", "x-forwarded-host", "forwarded", "x-mission-control-client-ip"]) delete sourceHeaders[name];
  ...
  if (trusted) {
    ...
    if (clientIp) sourceHeaders["x-mission-control-client-ip"] = clientIp;
  }
