...
test("untrusted requests lose spoofed forwarding headers and host", () => {
  ...
  const request = { socket: { remoteAddress: "192.168.86.99" }, headers: { host: "evil.example", "x-forwarded-proto": "https", "x-forwarded-for": "1.2.3.4", forwarded: "proto=https" } };
...
test("trusted proxy keeps verified HTTPS and client chain", () => {
  ...
  assert.deepEqual(request.headers, { host: "hera.connect2home.nl", "x-forwarded-proto": "https", "x-forwarded-for": "203.0.113.10" });
});
